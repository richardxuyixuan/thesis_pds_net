...
import math
from tasks.semantic.modules.SalsaNextCompletion import SalsaNextCompletion
...
class SalsaNextCompletionStageTwoFilteredWeights(nn.Module):
    def __init__(self, nclasses):
        super(SalsaNextCompletionStageTwoFilteredWeights, self).__init__()
        self.nclasses = nclasses
        self.completion_network = SalsaNextCompletion(nclasses)
        ...
        self.uf_for_mask = nn.Unfold(kernel_size=5, padding=(5-1)*1//2, dilation=1)

        # self.completor = continuous_convolution_network(9, 9)
    def forward(self, x, proj_mask):
        b, _, h, w = x.shape

        _, completed_scene = self.completion_network(x, proj_mask)
        uf_mask = self.uf_for_mask(proj_mask.float().unsqueeze(1)).reshape(b, 1, 5**2, h, w).sum(2)
        uf_mask = torch.clip(uf_mask, max=1)
        completed_scene *= uf_mask
        ...
        return logits_s, completed_scene, uf_mask