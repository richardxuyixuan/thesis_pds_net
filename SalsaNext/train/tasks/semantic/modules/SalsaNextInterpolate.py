...
class SalsaNextInterpolate(nn.Module):
    def __init__(self, nclasses):
        super(SalsaNextInterpolate, self).__init__()
    ...
    def forward(self, x, proj_mask):
        ...
        completed_scene = self.uf(x).reshape(b, c, self.kernel_size**2, h, w).sum(2)
        uf_mask = self.uf(proj_mask.float().unsqueeze(1)).reshape(b, 1, self.kernel_size**2, h, w).sum(2)
        uf_mask = torch.clip(uf_mask, min=1)
