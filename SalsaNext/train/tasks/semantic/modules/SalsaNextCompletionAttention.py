...
class continuous_convolution_network(nn.Module):
    def __init__(self, search_kernel, k_neighbour, dilation=1):
        ...
        self.post_linear = nn.Sequential(
            nn.Linear(self.mlp_output * self.k_neighbour, self.mlp_output),
            nn.BatchNorm1d(self.mlp_output),
            nn.ReLU(),
        )

    def forward(self, input_coords, input_feats):
        b, c, h, w = input_coords.shape
        _, f, _, _ = input_feats.shape
        ...
        uf_coords = self.uf(input_coords).reshape(b, c, self.kernel_size**2, h, w)
        mask = torch.sum(uf_coords[:,0], dim=1) > 0 # a mask to determine if neighbourhoods have any projected points
        uf_coords = (uf_coords - uf_coords[:,:,[self.kernel_size ** 2//2],...]).abs() # get relative distance

        # obtain features to be considered
        uf_feats = self.uf(input_feats).reshape(b, f, self.kernel_size**2, h, w)

        uf_coords = uf_coords.permute(0,3,4,1,2)[mask] # shape: [num_entries, 5, k]
        uf_feats = uf_feats.permute(0, 3, 4, 1, 2)[mask]  # shape: [num_entries, 5, k]

        # find closest neighbours
        uf_distance = uf_coords[:,0] + uf_coords[:,-1] + 0.01 # 0.01 is added so that the gaussian kernel can still apply to zero-location
        inv_gauss_k = (1 - get_gaussian_kernel(uf_distance, self.kernel_size, 10, 1)).view(1, -1)
        uf_distance = uf_distance * inv_gauss_k
        ...
        uf_feats = torch.gather(
            input=uf_feats, dim=2, index=uf_distance_idx.unsqueeze(1).repeat(1,f,1)
        ) # shape: [num_non_zero_locations, f, num_neighbours]
        ...
        # apply attention weights
        uf_feats *= att
        uf_feats = uf_feats.view(uf_feats.shape[0], -1)

        uf_feats = self.post_linear(uf_feats)

        return_feats = torch.zeros((b,h,w,f), device=input_feats.device)
        return_feats[mask] = uf_feats
        return_feats = return_feats.permute(0,3,1,2)
...
class SalsaNextCompletionAttention(nn.Module):
    def __init__(self, nclasses):
        super(SalsaNextCompletionAttention, self).__init__()
        ...
        self.logitsc_final = nn.Conv2d(64, 5, kernel_size=(1, 1))

        self.attention_network = continuous_convolution_network(9, 9)
    def forward(self, x, proj_mask):
        ...
        downCntx += self.attention_network(x, downCntx)
        ...
        completed_scene = self.logitsc(up1ec)
        completed_scene = self.actc(completed_scene)
        completed_scene = self.bnc(completed_scene)
        completed_scene = self.logitsc_final(completed_scene)
