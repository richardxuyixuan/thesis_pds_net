...
class SalsaNextCompletionSingleDecoderMaskPredictionHead(nn.Module):
    def __init__(self, nclasses):
        ...
        self.logitsc_final = nn.Conv2d(64, 6, kernel_size=(1, 1))
        self.m = nn.Sigmoid()
    ...
    def forward(self, x, proj_mask):
        ...
        completed_mask = self.m(completed[:,-1])

        mask = (proj_mask -1) * (-1)
        completed_scene = completed[:,:-1] * mask.unsqueeze(1) + x

        return completed_scene, completed_mask