...
import math
from tasks.semantic.modules.SalsaNextCompletionSingleDecoderMaskPredictionHead import SalsaNextCompletionSingleDecoderMaskPredictionHead
...
class SalsaNextCompletionSingleDecoderMaskPredictionHeadStageTwo(nn.Module):
    def __init__(self, nclasses):
        super(SalsaNextCompletionSingleDecoderMaskPredictionHeadStageTwo, self).__init__()
        self.nclasses = nclasses
        self.completion_network = SalsaNextCompletionSingleDecoderMaskPredictionHead(nclasses)
    ...
    def forward(self, x, proj_mask):
        completed_scene, completed_mask = self.completion_network(x, proj_mask)
        completed_mask_thres = (completed_mask > 0.5).float().unsqueeze(1)
        completed_scene *= completed_mask_thres
