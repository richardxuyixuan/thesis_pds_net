...
class Carla(Dataset):
  ...
  def __getitem__(self, index):
    ...
    mask_file = scan_file.replace('in_vol', 'proj_mask')
    if self.simple:
        proj = np.load(scan_file)
        proj_mask = np.load(mask_file)
        proj_mask = torch.from_numpy(proj_mask).clone()[0]
        proj = torch.from_numpy(proj).clone()[0]
        proj = proj * proj_mask.float()

        if self.gt:
            proj_labels = np.load(label_file)
            proj_labels = torch.from_numpy(proj_labels).clone()[0]
            proj_labels = proj_labels * proj_mask
        else:
            proj_labels = torch.zeros((1))

        unproj_labels = torch.zeros((1))
        proj_x = torch.zeros((1))
        proj_y = torch.zeros((1))
        proj_range = torch.zeros((1))
        unproj_range = torch.zeros((1))
        proj_xyz = torch.zeros((1))
        unproj_xyz = torch.zeros((1))
        proj_remission = torch.zeros((1))
        unproj_remissions = torch.zeros((1))
        unproj_n_points = torch.zeros((1))
    elif 'nusc' in scan_file:
        ...
        if self.gt:
          ...
          if self.test:
            ...
            scan.sem_label = self.map(scan.sem_label.astype(int), self.learning_map)
            scan.proj_sem_label = self.map(scan.proj_sem_label.astype(int), self.learning_map)
        ...
        if self.gt:
          ...
          proj_labels_range = torch.from_numpy(scan.label_proj_range).clone()
          proj_labels_xyz = torch.from_numpy(scan.label_proj_xyz).clone()
          proj_labels_remission = torch.from_numpy(scan.label_proj_remission).clone()
          proj_labels_mask = torch.from_numpy(scan.label_proj_mask).clone()

          if self.test:
            proj_labels = torch.from_numpy(scan.proj_sem_label).clone()
            proj_labels = proj_labels * proj_labels_mask

          else:
            proj_labels = torch.zeros_like(proj_labels_range, dtype=int)

          proj_labels_input = torch.cat([proj_labels_range.unsqueeze(0).clone(),
                            proj_labels_xyz.clone().permute(2, 0, 1),
                            proj_labels_remission.unsqueeze(0).clone()])
          proj_labels_input = (proj_labels_input - self.sensor_img_means[:, None, None]
                  ) / self.sensor_img_stds[:, None, None]
          proj_labels_input = proj_labels_input * proj_labels_mask.float()

        else:
          proj_labels = []
          proj_labels_range = []
          proj_labels_xyz = []
          proj_labels_remission = []
          proj_labels_mask = []
          proj_labels_input = []
    ...
    # return
    return proj, proj_labels_input, proj_mask, proj_labels_mask, proj_labels, unproj_labels, path_seq, path_name, proj_x, proj_y, proj_range, unproj_range, proj_xyz, unproj_xyz, proj_remission, unproj_remissions, unproj_n_points
